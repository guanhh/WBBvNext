    ...
    public class CodeGenVM : BaseVM
    {
        ...
        public string VmDir
        {
            get
            {
                if (_vmdir == null)
                {
                    ...
                    vmdir = up.GetDirectories($"*.{PlugName}.ViewModel", SearchOption.AllDirectories).FirstOrDefault();

                    if (vmdir == null)
                        vmdir = up.GetDirectories().Where(x => x.Name.ToLower().EndsWith(".viewmodel")).FirstOrDefault();
        ...
        public string ShareDir
        {
            get
            {
                if (_sharedir == null)
                {
                    ...
                    sharedir = up.GetDirectories($"*.{PlugName}.Page", SearchOption.AllDirectories).FirstOrDefault();

                    if (sharedir == null)
                        sharedir = up.GetDirectories().Where(x => x.Name.ToLower().EndsWith(".shared")).FirstOrDefault();
        ...
        public string TestDir
        {
            get
            {
                if (_testdir == null)
                {
                    var up = Directory.GetParent(MainDir);
                    //优先查找插件目录
                    var testdir = up.GetDirectories($"*.{PlugName}.Test", SearchOption.AllDirectories).FirstOrDefault();

                    if (testdir == null)
                        testdir = new DirectoryInfo(MainDir);
        ...
        public string ControllerDir
        {
            get
            {
                if (_controllerdir == null)
                {
                    ...
                    if (sharedir == null)
                        sharedir = new DirectoryInfo(MainDir);

                    if (string.IsNullOrEmpty(Area))
                    {
                        _controllerdir = Directory.CreateDirectory(sharedir + $"{Path.DirectorySeparatorChar}Controllers").FullName;
                    }
                    else
                    {
                        _controllerdir = Directory.CreateDirectory(sharedir + $"{Path.DirectorySeparatorChar}Areas{Path.DirectorySeparatorChar}{Area}{Path.DirectorySeparatorChar}Controllers").FullName;
                    }
        ...
        public string ControllerNs
        {
            get
            {
                ...
                {
                    _controllerNs = MainNS + $".{PlugName}.Controllers";
                }
        ...
        public string DataNs
        {
            get
            {
                if (_dataNs == null)
                {
                    ...
                    {
                        _dataNs = MainNS + $".{PlugName}.DataAccess"; ;
                    }
                }
                return _dataNs;
            }
        ...
        public string GenerateTest()
        {
            ...
            if (TestDir != null)
            {
                ...
                else
                {
                    if (typeof(IBasePoco).IsAssignableFrom(modelType) || typeof(IPersistPoco).IsAssignableFrom(modelType))
                    {
                        rv = GetResource($"ApiTest.txt").Replace("$cns$", ControllerNs).Replace("$tns$", TestNs).Replace("$vns$", VMNs).Replace("$model$", ModelName).Replace("$mns$", ModelNS).Replace("$dns$", DataNs).Replace("$classnamel$", $"{ModelName}{(IsApi == true ? "Api" : "")}").Replace("$PlugName$", PlugName);
                    }
                    else
                    {
